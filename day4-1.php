...
foreach($numbers as $number) {
    $lastNumber = $number;
    foreach ($matrixes as $matrixPos => $matrix) {
        foreach ($matrix as $matrixNumberPos => $matrixNumber) {
            if ($number === $matrixNumber) {
                $matrixes[$matrixPos][$matrixNumberPos] = -1;
            }
        }
    }

    foreach ($matrixes as $matrixIndex => $matrix) {
        foreach ($matrix as $matrixNumbers) {
            for ($i = 0; $i < 5; $i++) {
                $rowSum = 0;
                for ($j = 0; $j < 5; $j++) {
                    $rowSum += $matrix[($i * 5) + $j];
...
$matrixSum = 0;
foreach ($matrixes[$bingo] as $matrixNumber) {
    if ($matrixNumber !== -1) {
        $matrixSum += $matrixNumber;
    }
}

var_export($matrixSum * $lastNumber);
